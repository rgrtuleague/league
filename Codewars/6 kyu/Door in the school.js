...
Description:
    ...
    Then pupils start coming. It might be hard to believe, but all of them want to study!
...
function doors(n) {
   ...
   var arr = new Array(n);
    ...
    for (var i=1;i<=n;i++) {
        for (var k=i-1; k < n;k+=i) {
            arr[k] = !arr[k];
        }
    }
    return arr.filter(function(x) {return x == true}).length;
}
